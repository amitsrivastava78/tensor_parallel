#!/usr/bin/env python3
"""
Clean Layer Types Tensor Parallelism Test
Properly isolated tests to prevent state contamination and ensure reproducible results.
"""
...
def create_mlp_model():
    ...
    # MLP layers with different sharding patterns
    x = layers.Dense(128, activation='relu', name="mlp_1")(inputs)  # Column-parallel
    x = layers.Dense(256, activation='relu', name="mlp_2")(x)      # Column-parallel
    x = layers.Dense(128, activation='relu', name="mlp_3")(x)      # Column-parallel
    outputs = layers.Dense(32, name="mlp_output")(x)                # Column-parallel
...
def test_single_model_tensor_parallelism(model_creator, model_name, input_shape, target_shape):
    """Test tensor parallelism for a single model in isolation."""
    print(f"\n🧪 Testing {model_name} Tensor Parallelism (Clean Environment)")
    print("=" * 70)
    ...
    # Test 3: Forward pass comparison (should be identical with mathematical identity mode)
    print(f"\n🔍 Test 3: Forward Pass Comparison")
    
    # Forward pass on single model (recreate to ensure clean state)
    single_model_clean = model_creator()
...
def main():
    """Run all layer type tests in clean isolation."""
    print("🚀 Clean Tensor Parallelism Layer Type Testing")
    print("Testing MLP, Self-Attention, and Einsum Dense Layers")
    print("=" * 80)
    
    setup_jax_backend()
    
    # Test each model type in complete isolation
    test_results = []
    
    # Test 1: MLP Model
    mlp_success = test_single_model_tensor_parallelism(
        create_mlp_model, "MLP Model", (32, 64), (32, 32)
    )
    ...
    # Test 2: Self-Attention Model
    attn_success = test_single_model_tensor_parallelism(
        create_self_attention_model, "Self-Attention Model", (16, 32, 64), (16, 16)
    )
    ...
    # Test 3: Einsum Dense Model
    einsum_success = test_single_model_tensor_parallelism(
        create_einsum_dense_model, "Einsum Dense Model", (24, 48), (24, 24)
    )
    ...
    if all_passed:
        ...
        print("🎉 Tensor parallelism is working correctly with proper isolation!")
    else:
        print("🎯 Overall Result: ❌ SOME TESTS FAILED")
        print("🔧 Need to investigate failing tests further.")
